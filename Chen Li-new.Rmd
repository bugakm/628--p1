...
table(bjweek$Value[bjweek$Value<=0])
bjweek$Value[bjweek$Value==-1]<-0
bjweek$Value[bjweek$Value==-2]<-0
bjweek$Value[bjweek$Value==-15]<-0
table(bjweek$Value[bjweek$Value<=0])
```
...
dataclean2<-function(data){
    newdata<-data %>% 
    filter(Value<0) %>%
    group_by(Month,Day) %>%
    mutate(mean_Value_Day = NA) %>%
    ungroup()
    return(newdata)
}
bjclean1<-dataclean1(bjweek)
bjclean2<-dataclean2(bjweek)
bjclean2$Value<-NA
...
bjclean$week<-as.factor(bjclean$week)
```
